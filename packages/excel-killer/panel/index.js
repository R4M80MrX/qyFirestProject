    ...
    ready() {
        ...
        window.plugin = new window.Vue({
            ...
            methods: {
                ...
                _getJavaScriptSaveData(excelData, itemSheet, isClient , jsAllData = {}) {
                   
                    let title_count = 1;
                    let sheetFormatData = {};                    
                    //ccxx ggff //处理复杂化excel
                    //去# 判断title层数
                    for(let i = 0;i<title_count;i++)
                    {
                        let title = excelData[i];
                        for (let j = 1; j < title.length; j++) {
                            let key = title[j];
                            if(typeof(key) == "string" && key.length > 0 && key[key.length-1] == '#')
                            {
                                title[j] = key.substr(0,key.length - 1);
                                title_count = i + 2;
                            }
                        }
                    }
                    //补充title
                    for(let i = title_count - 2;i >=0 ;i--)
                    {                
                        if(excelData[i].length > 0)
                        for( let j = 1; j < excelData[i+1].length;j++)
                        {
                            for( let k = 0; k <=i; k++)
                            {
                                if(excelData[k][j] != undefined)
                                {
                                    break;
                                }
                                if(k == i)
                                {
                                    excelData[i][j] = excelData[i][j-1];
                                }
                            }
                        }
                    }
                    ...
                    let desc = excelData[title_count];
                    let target = excelData[title_count + 1];
                                      

                    for (let i = title_count + 2; i < excelData.length; i++) {
                        ...
                        for (let j = 1; j < title.length; j++) {
                            ...
                            if (canExport) {                                
                                ...
                                if (value === undefined) {
                                    value = "";
                                    this._addLog(JSON.stringify(itemSheet));
                                    this._addLog("[Error] 发现空单元格:" + itemSheet.name + "*" + itemSheet.sheet + " => (" + key + "," + (i + 1) + ")");
                                }
                                
                                if(title_count == 1 && title.length == 2 && title[1].toLowerCase() == "value")
                                {
                                    //ccxx ggff 数组
                                    saveLineData = value;
                                }
                                ...
                                {
                                    //ccxx ggff 处理复杂化
                                    let data = saveLineData;

                                    for( let k = 1;k < title_count ; k++)
                                    {
                                        let key_child = excelData[k][j];
                                        //this._addLog("j " + j + " child " + key_child );
                                        if(key_child != undefined)
                                        {
                                            //数组处理
                                            if(key_child.toLowerCase() == "excelarray")
                                            {
                                                if(jsAllData && jsAllData[value] != undefined)
                                                {
                                                    value = jsAllData[value];
                                                }
                                                else
                                                {
                                                    value = {};
                                            ...
                                            {
                                                let key_parent = excelData[k -1][j];                                            
                                                if(!data[key_parent])
                                                {
                                                    data[key_parent] = {};
                                                }
                                                data = data[key_parent];
                ...
                onBtnClickGen() {
                                ...
                                if (sheetData.length > 3) {
                                    ...
                                    if (this.isExportJs) {
                                        ...
                                        let writeFileJs = function (savePath, isClient) {
                                            let excel_name = itemSheet.name;
                                            let name_index = itemSheet.name.lastIndexOf(".xls");                                   
                                            if(name_index > 0)
                                            {
                                                excel_name = excel_name.substr(0,name_index);
                                            }

                                            let sheetJsData = this._getJavaScriptSaveData(sheetData, itemSheet, isClient , isClient ? jsAllSaveDataClient[excel_name]:jsAllSaveDataServer[excel_name]);
                                            if (Object.keys(sheetJsData).length > 0) {
                                                //ccxx ggff 用excel名字做开头                                              

                                               // if (this.isMergeJavaScript) {
                                                 
                                                    if (isClient) {
                                                        if(jsAllSaveDataClient[excel_name] === undefined)
                                                        {
                                                            jsAllSaveDataClient[excel_name] = {};
                                                        }

                                                        // 检测重复问题
                                                        if (jsAllSaveDataClient[excel_name][itemSheet.sheet] === undefined) {
                                                            if(itemSheet.sheet.toLowerCase() == "this")
                                                            {
                                                                jsAllSaveDataClient[excel_name] = Object.assign(jsAllSaveDataClient[excel_name], sheetJsData);
                                                            }
                                                            else if(itemSheet.sheet.toLowerCase() == "delete")
                                                            {
                                                                for(var key in sheetJsData)
                                                                {
                                                                    jsAllSaveDataClient[excel_name][key] = undefined;
                                                                }
                                                            }
                                                            else
                                                            {
                                                                jsAllSaveDataClient[excel_name][itemSheet.sheet] = sheetJsData;
                                                            }
                                                        } else {
                                                            this._addLog("发现重名sheet:" + itemSheet.name + "(" + itemSheet.sheet + ")");
                                                        }
                                                    } else {
                                                        if(jsAllSaveDataServer[excel_name] === undefined)
                                                        {
                                                            jsAllSaveDataServer[excel_name] = {};
                                                        }

                                                        // 检测重复问题
                                                        if (jsAllSaveDataServer[excel_name][itemSheet.sheet] === undefined) {
                                                            if(itemSheet.sheet == "this")
                                                            {
                                                                jsAllSaveDataServer[excel_name] = Object.assign(jsAllSaveDataServer[excel_name], sheetJsData);
                                                            }
                                                            else
                                                            {
                                                                jsAllSaveDataServer[excel_name][itemSheet.sheet] = sheetJsData;
                                                            }
                                                        } else {
                                                            this._addLog("发现重名sheet:" + itemSheet.name + "(" + itemSheet.sheet + ")");
                                                        }
                    ...
                    if (this.isExportJs) {
                        if(this.isMergeJavaScript)
                        {
                            ...
                            //不合并
                            if (this.isExportClient) {
                                for(var key in jsAllSaveDataClient)
                                {
                                    let fileNameFullPath = path.join(jsSavePath1, key + ".js");
                                    this._onSaveJavaScriptCfgFile(fileNameFullPath, jsAllSaveDataClient[key]);
                                }                                
                            }
                            if (this.isExportServer) {
                                for(var key in jsAllSaveDataServer)
                                {
                                    let fileNameFullPath = path.join(jsSavePath2, key + ".js");
                                    this._onSaveJavaScriptCfgFile(fileNameFullPath, jsAllSaveDataServer[key]);
                                }                                
                            }
